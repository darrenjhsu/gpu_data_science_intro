...
JUPYTER_APPTAINER := bash -c "pip install jupyterlab_nvdashboard && jupyter lab --ip=0.0.0.0 --allow-root --no-browser --notebook-dir=$(PWD) --NotebookApp.allow_origin='*'"
ifdef NV_GPU
...
bruno: | /hpc/mydata/greg.zynda/gpu_ds/pytorch_25.03-py3.sif
	apptainer exec --nv /hpc/mydata/greg.zynda/gpu_ds/pytorch_25.03-py3.sif $(JUPYTER_APPTAINER)
